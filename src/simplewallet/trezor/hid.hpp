...
namespace simplewallet
{
  namespace trezor
  {
    namespace hid
    {
      ...
      {
        return hid_executor->await([=] { return hid_enumerate(vendor_id, product_id); });
      }

      void free_enumeration(hid_device_info *devs)
      {
        return hid_executor->await([=] { return hid_free_enumeration(devs); });
      }
      ...
      {
        return hid_executor->await([=] { return hid_write(device, data, length); });
      }
